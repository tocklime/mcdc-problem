...
newtype TestBE = TestBE {unTest :: BoolExp Char} deriving(Show)
instance Arbitrary TestBE where
  arbitrary = TestBE <$> sized exp'
    where
      exp' n | n <= 0 = Lit <$> arbitrary
             | otherwise = 
                oneof [ Lit <$> arbitrary
                      , Not <$> sub
                      , Or <$> sub <*> sub
...
spec = do
  ...
  describe "safe unions" $ do
    it "merges distinct key sets" $ safeUnion (M.fromList [('a','a')]) (M.fromList [('b','b')]) `shouldBe` Just (M.fromList [('a','a'),('b','b')])
    it "merges like keys with like values sets" $ safeUnion (M.fromList [('a','a')]) (M.fromList [('a','a')]) `shouldBe` Just (M.fromList [('a','a')])
    it "fails when there are like keys with different values" $ safeUnion (M.fromList [('a','a')]) (M.fromList [('a','b')]) `shouldBe` Nothing
  describe "MCDC generator" $ do
    prop "the test cases generated return the given values" $ \(TestBE e) ->
      case findMcdc e of
        Left _ -> property Discard
        Right a -> property $ all (\m -> eval (\c -> m M.! c) e) (S.toList (trues a)) 
